...
export type PdfBrand = {
  ...
  tagline: string
  synopsis?: string
  loadLogo?: () => Promise<Uint8Array | null>
}
...
function addPdfBrandHeader(doc: jsPDF, y: number, margin: number, brand: PdfBrand, logoBytes: Uint8Array | null): number {
  ...
  doc.setFontSize(12)
  doc.setTextColor(0)
  doc.text(pdfText(brand.name), textX, y + 5)

  doc.setFont('helvetica', 'italic')
  doc.setFontSize(9)
  doc.setTextColor(80)
  doc.text(pdfText(brand.tagline), textX, y + 10)
  doc.setTextColor(0)
...
export async function downloadPdfReport(
  ...
  options: { brand: PdfBrand; filename: string },
): Promise<void> {
  const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'letter' })
  const pageWidth = doc.internal.pageSize.getWidth()
  const logoBytes = (await options.brand.loadLogo?.()) ?? null
  let y = PDF_MARGIN

  y = addPdfBrandHeader(doc, y, PDF_MARGIN, options.brand, logoBytes)
  y += 2
  ...
  if (options.brand.synopsis) {
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(10)
    const synopsisLines = doc.splitTextToSize(pdfText(options.brand.synopsis), pageWidth - PDF_MARGIN * 2)
